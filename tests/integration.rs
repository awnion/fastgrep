...
/// Binary files (containing NUL) should output "binary file matches" to stderr,
/// not dump binary content to stdout.
#[test]
fn binary_file_detected() {
    ...
    // stdout should be empty (binary file, no lines printed)
    assert!(output.stdout.is_empty(), "binary file should not produce stdout");
    // stderr should contain "binary file matches"
...
/// Binary file with -l should print filename to stdout (not the binary message).
#[test]
...
/// Binary file with -c should print count to stdout.
#[test]
fn binary_file_with_count() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    // -c on binary file should report count (at least 1 since file matches)
    assert!(output.status.success());
    assert!(!stdout.is_empty(), "count mode should produce output for binary file");
}

/// Non-binary file should NOT produce binary file message.
#[test]
...
/// NUL byte deep in file should still trigger binary detection.
#[test]
fn binary_detection_nul_at_end() {
    let mut f = NamedTempFile::new().unwrap();
    // Write lots of text, then a NUL near the end
    for _ in 0..1000 {
